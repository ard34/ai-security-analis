...
def test_ui_requirements_skip_streamlit_stack_on_python_315():
    content = read("requirements-ui.txt")
    requirement_lines = [
        line.strip() for line in content.splitlines() if line.strip() and not line.strip().startswith("#")
    ]

    assert 'streamlit>=1.30; python_version < "3.15"' in content
    assert 'reportlab>=4.0; python_version < "3.15"' in content
...
def test_docs_explain_python_315_ui_dependency_fallback():
    ...
    assert "python 3.15" in combined
