...
def defensive_recommendation_for(finding: Finding) -> str:
    ...
    if "missing security header" in title or "header" in title:
        return "Manually verify the response headers in an authorized browser session and confirm expected defensive headers are configured."
    if "auth" in title:
        return "Review the authorization design and confirm access control decisions with code owners using approved test accounts."
    if "secret" in title or "token" in title:
        return "Confirm whether the exposed value is a real secret, rotate it if needed, and remove it from source history through the approved process."
